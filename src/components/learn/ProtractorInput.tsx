...
const RADIUS = 110;
const CX = RADIUS + 10; // centre x within the protractor SVG viewbox
const CY = RADIUS + 10; // centre y within the protractor SVG viewbox
const VIEW_W = RADIUS * 2 + 20;
const VIEW_H = RADIUS + 20;

/** Build SVG arc path for the semicircle */
...
/** Convert polar (angle from baseline, radius) to cartesian relative to protractor centre */
function polar(angleDeg: number, r: number) {
  // 0° = right along baseline; angles measured anticlockwise from baseline
...
/** Render the graduated semicircle SVG (no interactivity — just markings) */
function ProtractorFace() {
  ...
  return (
    <>
      ...
      {/* Ticks and labels */}
      {ticks}
      {/* Rotation drag handle — small circle at 0° end of baseline */}
      <circle
        cx={CX + RADIUS}
        cy={CY}
        r="7"
...
export function ProtractorInput({ config, value, onChange }: ProtractorInputProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const svgRef = useRef<SVGSVGElement>(null);

  // Position of the protractor centre within the container (px)
  const [pos, setPos] = useState({ x: 160, y: 160 });
  // Rotation in degrees (how much the baseline is rotated from horizontal)
  ...
  // Render the SVG protractor as an absolutely positioned overlay
  const svgSize = { w: VIEW_W, h: VIEW_H };
  // Scale factor: we render the SVG at 240px wide on screen
  const displayW = 240;
    ...
    <div className="space-y-4">
      ...
      <div className="anx-callout-info text-sm">
        ...
        <ol className="list-decimal pl-5 space-y-0.5" style={{ color: 'var(--anx-text-secondary)' }}>
          <li>Drag the <strong>body</strong> of the protractor to place its centre on the angle vertex.</li>
      ...
      >
        ...
        {/* Draggable / rotatable protractor SVG */}
        <svg
          ref={svgRef}
