...
export interface FocusArea {
  ...
  label: string;
  description: string;
  /** Skill this focus area maps to, when known — lets the UI deep-link to practice. */
  skillId: string | null;
}
...
export interface StudentSessionSummary {
  ...
  correctCount: number;
  partialCount: number;
  laneJourney: Array<{ fromLane: string | null; toLane: string; transitionType: string }>;
...
/**
 * Builds the per-student post-lesson summary: attempt totals, per-skill
 * breakdown, lane journey, strengths (skills ≥80%), and focus areas (every
 * misconception hit, plus skills under 60% accuracy). Shared by the
 * `my-summary` review endpoint and the dashboard "keep working on" card so the
 * two never diverge. Returns null when the student wasn't in the session.
 */
export async function summariseStudentSession(
  sessionId: string,
  userId: string,
): Promise<StudentSessionSummary | null> {
  const [participant, liveSession] = await Promise.all([
    prisma.liveParticipant.findUnique({
      where: { liveSessionId_studentUserId: { liveSessionId: sessionId, studentUserId: userId } },
  ...
  if (!participant || !liveSession) return null;
  ...
  const [attempts, transitions] = await Promise.all([
    prisma.liveAttempt.findMany({
      ...
      select: { skillId: true, correct: true, markingResult: true, misconceptionId: true, createdAt: true },
  ...
  const misconceptionSkill = new Map<string, string>();
  ...
  for (const attempt of attempts) {
    ...
    if (outcome !== 'correct' && attempt.misconceptionId) {
      misconceptionSkill.set(attempt.misconceptionId, attempt.skillId);
    }
  }
  ...
  const skillBreakdown: SkillBreakdownEntry[] = [...skillAgg.entries()].map(([skillId, agg]) => ({
    skillId,
  ...
  for (const [misconceptionId, skillId] of misconceptionSkill) {
    const meta = mcLabelMap.get(misconceptionId);
    if (meta) focusAreas.push({ kind: 'misconception', label: meta.label, description: meta.description, skillId });
  }
