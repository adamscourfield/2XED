...
export default async function AdminContentPage({ params }: Props) {
  ...
  if (!subject) {
    return (
      <AdminPageFrame title="Subject not found" subtitle={`No subject with slug “${subjectSlug}”.`}>
        <p className="text-sm" style={{ color: 'var(--anx-text-muted)' }}>
          Check the URL or return to the admin home.
        </p>
      </AdminPageFrame>
    );
  }
  ...
  return (
    <AdminContentQaView subjectTitle={subject.title} subjectSlug={subject.slug} qaItems={qaItems} />
  );
}
