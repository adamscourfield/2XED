...
// Skill metadata for auto-creation — keeps the import self-contained so skills
// do not need to exist in the DB before questions can be imported.
const SKILL_META: Record<string, { name: string; strand: string; sortOrder: number }> = {
  ...
  'N1.17': { name: 'Write 10, 100, 1000 etc. as powers of 10', strand: 'POW',  sortOrder: 140 },
  'N1.18': { name: 'Write positive integers in the form A × 10^n', strand: 'POW',  sortOrder: 150 },
  'N1.19': { name: 'Understand negative powers of 10', strand: 'POW',  sortOrder: 160 },
  'N1.20': { name: 'Place value systems beyond base 10', strand: 'PV',   sortOrder: 170 },
  'N1.21': { name: 'Extended number-line reasoning (scaled intervals and estimation)', strand: 'PV',   sortOrder: 171 },
  'N1.22': { name: 'Directed numbers in context using linear scales', strand: 'PV',   sortOrder: 172 },
  'N1.23': { name: 'Multi-step comparison and ordering across representations', strand: 'PV',   sortOrder: 173 },
  'N1.24': { name: 'Synthesis: integers, decimals, negatives, and powers of ten on a line', strand: 'PV',   sortOrder: 174 },
...
async function main() {
  ...
  for (const filePath of packFiles) {
    ...
    for (const line of lines) {
      ...
      // ORDER contract errors: downgrade to SHORT_TEXT
      ...
      if (!skill) {
        console.warn(`  [SKIP] ${row.source.question_ref}: skill ${skillCode} not in SKILL_META (add it or run db:seed)`);
        skipped += 1;
        continue;
      }
      ...
      const visuals = resolveItemVisuals(
        { question: row.question.stem, options: derived.options },
        row.skills.primary_skill_code
      );

      const questionText = `[${row.source.question_ref}] ${row.question.stem}`;

      // Build misconceptionMap from Codex-format misconceptions array
      const misconceptionMap =
        row.misconceptions && row.misconceptions.length > 0
          ? row.misconceptions.reduce<Record<string, string>>((acc, m) => {
      ...
      const options = {
        ...
        meta: {
          question_role: roleFromNotes(row.variation?.notes),
          cognitive_load_level: row.cognitive_load?.level ?? null,
          quality_status: row.metadata?.quality_status ?? null,
          mapper: row.metadata?.mapper ?? null,
          source: {
            question_ref: row.source.question_ref,
            source_file: row.source.source_file ?? null,
            slide_number: slideNumber,
            tier: row.source.tier ?? null,
          },
        },
      };
  ...
  // Archive placeholder items for skills that now have real questions
