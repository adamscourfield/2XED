...
export function numericDistractors(answer: string): string[] {
...
export async function runUnit1Import(config: ImportConfig) {
  const prisma = new PrismaClient();
  try {
    const mappingPath = path.resolve(process.cwd(), config.mappingRelativePath);
    ...
    console.log(`✅ ${config.label} import complete. Created: ${created}, Updated: ${updated}, Total lines: ${lines.length}`);
  } finally {
