...
type MappingRow = {
  source: { question_ref: string };
  question: { stem: string; answer: string; options?: string[] };
  skills: { primary_skill_code: string; secondary_skill_codes?: string[] };
  marking?: { accepted_answers?: string[] };
};
...
function optionsForQuestion(stem: string, answer: string): string[] {
  ...
  if (lcAnswer === 'true' || lcAnswer === 'false') return ['True', 'False'];
  const numeric = numericDistractors(answer);
  if (numeric.length >= 3) return [answer, ...numeric];
...
async function main() {
  const mappingPath = path.resolve(process.cwd(), 'docs/unit-mapping/review-pack-unit1-partA-foundation-gapfill-n11-n12-n15.jsonl');
  ...
  for (const line of lines) {
    ...
    if (!item) {
      ...
      item = await prisma.item.update({ where: { id: item.id }, data: { options, answer, type: 'MCQ' } });
      updated += 1;
    }
  ...
  console.log(`✅ Unit1 gapfill (n11/n12/n15) import complete. Created: ${created}, Updated: ${updated}, Total lines: ${lines.length}`);
}
