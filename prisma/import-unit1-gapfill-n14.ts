...
function optionsForQuestion(stem: string, answer: string): string[] {
  ...
  if (lcStem.includes('equivalent to')) return [answer, '1 > 2', '2 < 1', '1 = 2'];

  if (lcStem.includes('at least')) return [answer, '? > 7', '? ≤ 7', '? < 7'];

  const numeric = numericDistractors(answer);
  if (numeric.length >= 3) return [answer, ...numeric.slice(0, 3)];
...
async function main() {
  const mappingPath = path.resolve(process.cwd(), 'docs/unit-mapping/review-pack-unit1-partA-foundation-gapfill-n14.jsonl');
  ...
  console.log(`✅ Unit1 gapfill n14 import complete. Created: ${created}, Updated: ${updated}, Total lines: ${lines.length}`);
}
